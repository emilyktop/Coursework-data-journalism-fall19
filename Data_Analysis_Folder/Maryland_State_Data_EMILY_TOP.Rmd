...
rural_urban_codes <- read_csv("ruralurbancodes2013.csv")qa
```
...
It appears that in Maryland, the number of deaths due to opioids in not higher in rural areas than it is in urban areas. What about when we look at it based on Maryland's definition of rural v urban?

```{r}
#join death information with maryland information

maryland_opioids <- maryland_rural_urban2 %>%
  inner_join(opioid_deaths, by=c("countyfips.x"="county_code"))

maryland_opioids %>%
  group_by(countyfips.x, rural_or_urban) %>%
  summarise(county_total_pop = sum(population.x), county_total_deaths = sum(deaths)) %>%
  filter(!is.na(county_total_pop), !is.na(county_total_deaths)) %>%
  group_by(rural_or_urban) %>%
  summarise(group_total_pop = sum(county_total_pop), group_total_deaths = sum(county_total_deaths)) %>%
...
```
If Maryland's usage or rural v uran is considered, urban areas still have a higher deaths per pop rate, but it closes slightly.
...
md_pharm_tracts <- maryland_pharmacies %>%
  inner_join(acros_tracts, by=c("BUYER_DEA_NO"="buyer_dea_no"))
...
#combine nytimes tracts to mdpharmtractspills

md_pharm_tracts_pills_labels <- md_pharm_tracts_pills %>%
...
md_pharm_tracts_pills_labels %>%
  group_by(county_fips, tract_type) %>%
...
^^ check the above b/c population is just 2010 population... 

If the above analysis is correct, places in the inner ring have the highest pills per person compared to all other places. The inner ring are denser suburbs and outer ring is less dense suburbs. 
