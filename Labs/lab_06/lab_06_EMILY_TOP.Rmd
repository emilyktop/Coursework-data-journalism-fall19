...
```{r}
#Create subset of mingo county data
...
mingo_county_pills_per_year <- arcos_county_pills_per_year %>%
 ...
 # This is the final version of this graphic
  ...
  geom_bar(stat="identity", aes(year, dosage_unit), fill="royal blue") +
  labs(x="Year", y="Total pills", title="In Mingo County, pills spiked in 2008, then dropped significantly in 2010", subtitle = "Total pills shipped to Mingo County by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma)

  # Also saves a web version
  ggsave("mingo_county_plot.png", plot=last_plot())
...
```{r}
#create 2012 pills subset for West Virginia
west_virginia_pills_2012 <- arcos_county_pills_per_year %>%
  filter(buyer_state == "WV", year=="2012") %>%
  select(buyer_county, year, dosage_unit)

#Create population subset for West Virginia in 2012
west_virginia_population_2012 <- county_population(key = key) %>%
  clean_names() %>%
  filter(buyer_state == "WV", year=="2012") %>%
  select(buyer_county, population)

#Combine pills and population data for West Virginia in 2012
west_virginia_2012 <- west_virginia_pills_2012 %>%
  inner_join(west_virginia_population_2012, by=("buyer_county")) %>%
  select(buyer_county, dosage_unit, population)

#Create pills to population plot for West Virginia in 2012
ggplot(west_virginia_2012) +
  geom_point(aes(population, dosage_unit)) +
  labs(x="2012 Population", y="Total pills in 2012", title="Kanawha, Raleigh, and Cabell had high number of pills relative to population", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(population, dosage_unit), method = "lm", se = FALSE) +
  geom_text_repel(aes(population, dosage_unit, label=buyer_county), subset(west_virginia_2012, population > 75000))
