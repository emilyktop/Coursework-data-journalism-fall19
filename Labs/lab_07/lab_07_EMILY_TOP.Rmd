...
```{r}
# Glimpse at not_in_population table
glimpse(not_in_population)
#view(not_in_population)

#The majority of the counties that are not in the population dataset are areas that are tied to the U.S. but are not states, for example, Guam and Puerto Rico. This could cause problems if there are shipments to these areas, and I want to look at the pills per person data, I would not be able to do so. 
```
...
state_geodata_shifted %>%
  ...
  labs(fill='Population',title="California, Texas have largest populations", subtitle = "2017 population, U.S. Census", caption = "Source: U.S. Census ACS") +
  theme(legend.position="right") +
  scale_fill_viridis_c(option = "cividis",labels = comma)
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='pills_per_person',title="West Virginia, Kentucky, and South Carolina had highest pills per population in 2009", subtitle = "2009 pills per person, U.S. Census and ARCOS data", caption = "Source: U.S. Census ACS") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills per person grew between 2006 and 2012", subtitle = "2006 thru 2012 pills per person, U.S. Census and ARCOS data", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
pills_population_county_2009 <- pills_population %>%
  ...
  summarise(county_pills=sum(dosage_unit, na.rm = TRUE),
            county_population=sum(population, na.rm=TRUE)) %>%
  mutate(pills_per_person=county_pills/county_population)

geo_pills_population_county_2009 <- county_geodata_shifted %>%
 inner_join(pills_population_county_2009, by=c("GEOID" = "countyfips"))

geo_pills_population_county_2009 %>%
  ggplot(aes(fill = pills_per_person)) +
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='pills_per_person',title="Mid-Atlantic rural regions, such as in West Virginia, had higher pills per person in 2009", subtitle = "2009 pills per person, U.S. Census and ARCOS data", caption = "Source: U.S. Census ACS") +
  theme(legend.position="bottom") +
